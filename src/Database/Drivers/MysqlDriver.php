...
namespace Fw\Database\Drivers;

use Fw\Exception\Exception;
use Fw\Database\DatabaseException;
use Fw\Instance\InstanceTrait;
use Fw\Database\DatabaseAbstract;

class MysqlDriver extends DatabaseAbstract
{
    use InstanceTrait {
...
EOT;
    ...
    protected function _execute($sql, $params)
    {
        ...
        try {
            ...
            $DatabaseException = new DatabaseException($e->getMessage(), $e->getCode());
            $DatabaseException->setPrepareSql($sql);
            $DatabaseException->setParams($params);
            $config = $this->getConfig($this->type);
            $DatabaseException->setHost($config['host']);
            $DatabaseException->setPort($config['port']);
